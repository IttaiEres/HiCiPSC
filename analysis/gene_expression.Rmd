...
enrichment.plotter(gene.hic.filt, "adj.P.Val", "min_FDR.H", "Minimum DE FDR of Genes Overlapping Hi-C Contacts", xmax=1, recip=TRUE) #FIGS19A/B
...
#FIGS19C/D
enrichment.plotter(gene.hic.filt, "weighted_Z.ALLvar.H", "adj.P.Val", "FDR for Weighted p-val Combine of Hi-C Contacts Overlapping Gene", xmax=1) #FIGS19C/D
...
#FIGS19
FIGS19A <- pap.enrichment.plotter(gene.hic.filt, "adj.P.Val", "min_FDR.H",  "Minimum FDR of Genes", xmax=1, recip=TRUE) #FIGS19A
FIGS19B <- pap.enrichment.plotter(gene.hic.filt, "adj.P.Val", "min_FDR.H",  "Minimum FDR of Genes", xmax=1, significance = TRUE, recip=TRUE) #FIGS19B
FIGS19 <- plot_grid(FIGS19A, FIGS19B, labels=c("A", "B"), align="h", rel_widths=c(1, 1.2))
save_plot("~/Desktop/FIGS19.png", FIGS19, nrow=1, ncol=2)
```
...
design <- model.matrix(~meta.data$SP+meta.data$SX)
...
#Also test with log2 CPM instead of RPKM:
exprs.2 <- as.data.frame(lcpms)
exprs.2$genes <- rownames(exprs.2)
exprs.2 <- exprs.2[,c(8, 6, 1, 4, 7, 5, 2, 3, 9)]
exprs.2 <- exprs.2[(which(exprs.2$genes %in% exprs$genes)),]

genes <- exprs$genes
genes2 <- exprs.2$genes
DEgenes <- filter(hmin, adj.P.Val<=0.05) %>% select(., genes)
myDEindices <- which(genes %in% DEgenes[,1])
myDEindices.2 <- which(genes2 %in% DEgenes[,1])
exprs <- exprs[,-9]
exprs.2 <- exprs.2[,-9]
exprs <- data.matrix(exprs[present,])
exprs.2 <- data.matrix(exprs.2[present,])
hic <- data.matrix(hic[present,])
#designy <- rbind(as.vector(as.factor(c("human", "human", "chimp", "chimp", "human", "human", "chimp", "chimp"))), as.vector(as.factor(meta.data$SX)))
designy <- as.factor(c("human", "human", "chimp", "chimp", "human", "human", "chimp", "chimp"))
colnames(hic) <- colnames(exprs) <- colnames(exprs.2)
hic <- as.data.table(hic)
exprs <- as.data.table(exprs)
exprs.2 <- as.data.table(exprs.2)
...
#test <- mediate.test.regressing(exprs.2, designy, hic) Gives extremely similar results, 222/1401 genes significantly mediated (16%), and still 0 of non-DE genes.

#Shrink dat variance!
vshrDE <- vash(test$d_se[myDEindices], df=4)
...
summary(ash_reg_nonDE$result$svalue<=0.05)
...
#```{r, include=FALSE, echo=FALSE, Work for getting example regions. Useful for pyGenomeTracks}
# #Grab genes that were significantly mediated:
# therows <- which(ash_reg$result$svalue<=0.05)
# besthits <- myDEindices[therows]
# mygenes <- genes[besthits]
# 
# ###### START EXAMPLE
